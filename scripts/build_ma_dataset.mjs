...
// 5) Process per year (usually one)
...
if (!countyFips) {
  ...
  const contractKey = keyFromHeader(rows[0], ["Contract ID"]);
  const planKey = keyFromHeader(rows[0], ["Plan ID"]);
...
async function loadLandscapeRows(zipPath) {
  ...
  // 1) try CSVs first: find a CSV that contains the “Contract Year” & “County Name” etc.
  ...
  for (const e of csvCandidates) {
    ...
    const hasYear = headers.includes("contract year");
    const hasType = headers.includes("contract category type") || headers.includes("contract category");
    const hasCounty = headers.includes("county name") || headers.includes("county fips") || headers.includes("county code (fips)");
    const hasPlan = headers.includes("plan id") && headers.includes("contract id");
    if (hasYear && hasType && hasCounty && hasPlan) {
...
async function buildZipIndex() {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!z || !fips) continue;
    let m = freq.get(z);
    if (!m) { m = new Map(); freq.set(z, m); }
    m.set(fips, (m.get(fips) || 0) + 1);
  }
  const out = [];
  for (const [zip, m] of freq.entries()) {
    const total = Array.from(m.values()).reduce((a,b)=>a+b,0) || 1;
    const counties = Array.from(m.entries()).map(([fips,count]) => [fips, Math.round((count/total)*1000)/1000]);
